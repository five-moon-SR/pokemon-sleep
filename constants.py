...
# ---------------------------------------------------------------------------
EVALUATION_VERSION: str = "v1.5"  # 2026-07-24 スキルレベルアップの固定加点を廃止(過大評価回避・物理のスキル軸のみで評価)
...
OPTION_BONUS_SUBSKILL: dict[str, float] = {
    ...
    # スキルレベルアップS/M は固定加点を廃止（2026-07-24）。
    # 効果はスキル軸ブースト(_effective_skill_lv)で物理的に評価済みで、固定加点は
    # 二重計上（スキル型は三重）による過大評価だったため。物理効果のみで評価する。
}

# スキルレベルアップの「スキル型への固定上乗せ」は廃止（2026-07-24・過大評価回避）。
# 空 dict = 上乗せなし。物理のスキル軸ブーストのみで評価する。
...
# タイプ別「総合スコア%→ランク」閾値。
# 2026-07-23 独自キャリブレーション(scripts/calibrate_rank_thresholds.py)に切替。
# 背景: だいふく公式帯(下のDAIFUKU_*参照)は甘く、金1個体が軒並みSになっていた。
# スコア%自体はだいふくと一致することを実サイトで検証済み(リザードン61.44% vs 60.0%)。
# 品質アンカー(v1.4.1): C=無補正中央/B=並/A=並と良の中間/S=良(適正金1+スピM)中央/SS=優中央/増田=優上位10%。
RANK_THRESHOLDS_BY_TYPE: dict[int, list[tuple[float, str]]] = {
    1: [(101.0, "増田"), (99.0, "SS"), (76.5, "S"), (62.5, "A"), (48.5, "B"), (44.5, "C")],
    2: [(100.5, "増田"), (93.5, "SS"), (68.5, "S"), (57.0, "A"), (45.0, "B"), (41.0, "C")],
    3: [(100.5, "増田"), (93.5, "SS"), (68.5, "S"), (57.0, "A"), (45.0, "B"), (41.0, "C")],
    4: [(100.5, "増田"), (93.5, "SS"), (68.5, "S"), (57.0, "A"), (45.0, "B"), (41.0, "C")],
    5: [(81.5, "増田"),  (79.5, "SS"), (66.0, "S"), (56.0, "A"), (46.0, "B"), (42.0, "C")],
    6: [(100.5, "増田"), (93.5, "SS"), (68.5, "S"), (57.0, "A"), (45.0, "B"), (41.0, "C")],
    7: [(81.0, "増田"),  (79.0, "SS"), (66.5, "S"), (54.0, "A"), (41.5, "B"), (34.5, "C")],
    8: [(100.5, "増田"), (93.5, "SS"), (68.5, "S"), (57.0, "A"), (45.0, "B"), (41.0, "C")],
    9: [(93.5, "増田"),  (91.5, "SS"), (71.0, "S"), (54.0, "A"), (37.0, "B"), (29.5, "C")],
}

# 参考: だいふく公式のランク帯(旧採用値)。表示互換の確認用に残す。
DAIFUKU_RANK_THRESHOLDS_BY_TYPE: dict[int, list[tuple[float, str]]] = {
